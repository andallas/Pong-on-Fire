var Input = (function(Input)
{
	...
	Input.getPrevPressedKeys = function()
	{
		return _previouslyPressedKeys;
	}

	Input.initialize = function(theCanvas)
	{
		canvas = theCanvas;
		...
		canvas.addEventListener("DOMMouseScroll", handleMouseWheel_FF, false);			
	...
	{
		_mouseX = (e.pageX || e.clientX) - canvas.offsetLeft;
		_mouseY = (e.pageY || e.clientY) - canvas.offsetTop;
	}
	...
	function handleTouchStart(e)
	{
		...
		Input._mouseX = e.touches[0].pageX - Firestorm.canvas.offsetLeft;
		Input._mouseY = e.touches[0].pageY - Firestorm.canvas.offsetTop;
		//e.preventDefault();
	}
	...
	function handleTouchEnd(e)
	{
		...
		Input._mouseX = undefined;
		Input._mouseY = undefined;
		//e.preventDefault();
	}

	Input.preventDefaultKeys = function(arrayOfStrings)
	{
	...
	Input.pressed = function(keys, logicalAnd)
	{
		if(Utility.isString(keys))
		{
			keys = keys.split(" ");
		}
	...
	Input.pressedWithoutRepeat = function(keys, logicalAnd)
	{
		if(Input.pressed(keys, logicalAnd))
		{
